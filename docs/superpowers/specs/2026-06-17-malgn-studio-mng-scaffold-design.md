...
### 6.2 WBS 단계(stage) — 정본: `docs/planning/11-ROADMAP-WBS.md` §3

기획 로드맵에서 확정한 **9단계**를 D1 `stage` 시드 + `wbsData.ts` 메타로 사용(가중치 합 100, QA=0 표시용):
...
> WBS 초기 작업(`wbs_item`)은 **빈 상태로 시작**(화면 CRUD)하거나 11-ROADMAP-WBS §4 스타터 시드를 사용. 단계명·가중치는 planner가 실데이터로 재보정.
...
> **`docs/planning/` 서비스 기획 패키지는 이미 작성 완료**(00~12, 경쟁분석 반영). 아래는 관리 허브 운영 문서 골격으로, planning 패키지를 정본으로 참조한다.
